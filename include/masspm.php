...
if ($action == "post") {
    if (isset($_POST['masspm'])) {
        ...
        $ratio   = (isset($_POST["ratio"]) ? (int)$_POST["ratio"] : 0);
        $pick    = (isset($_POST["pick"]) ? (int)$_POST["pick"] : 0);
        ...
        if ($msg == "") {
            redirect("admincp.php?user=" . user::$current["uid"] . "&code=" . user::$current["random"] . "&do=masspm&action=write&error=return");
            exit();
        }
        //check if we want to PM selected userlevels
        if ($level > 0) {
            
            $user_id_query = $db->query("SELECT level FROM users_level WHERE id_level = " . $level) or sqlerr(__FILE__, __LINE__);
            $user_rank  = $user_id_query->fetch_array(MYSQLI_BOTH);
            $user_level = security::html_safe($user_rank['level']);
            
            $user_id_query1 = $db->query("SELECT level FROM users_level WHERE id_level = " . $level1) or sqlerr(__FILE__, __LINE__);
            $user_rank1  = $user_id_query1->fetch_array(MYSQLI_BOTH);
            $user_level1 = security::html_safe($user_rank1['level']);
        ...
        $i   = 0;
        //do database call
        $result_id = $db->query("SELECT * FROM users WHERE id > 1" . $where) or sqlerr(__FILE__, __LINE__);
        while ($id_collect = $result_id->fetch_array(MYSQLI_BOTH)) {
            if (!$list_users)
                $l_users = "not listing users as its deactivated";
            $user_id = (int)$id_collect['id'];
            // stop PM to sender added function below to PM sender ;)
            if ($user_id == user::$current['uid'])
                continue;
            //did we want to PM based on ratio?
            if ($check_ratio) {
                $downloaded = (int)$id_collect["downloaded"];
                $uploaded   = (int)$id_collect["uploaded"];
                ...
                if ($pick == 0) {
                    ...
                    if ($ratio == $ratio1) {
                        if ($list_users)
                            $l_users .= "<a href='userdetails.php?id=" . $user_id . "'>" . security::html_safe($id_collect['username']) . "</a>  -  ";
                        if ($pm)
                            $db->query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES (" . $sender . ", " . $user_id . ", UNIX_TIMESTAMP(), " . $subject . ", " . $msg . ")");
                    } else
                        continue;
                }
                //if ratio X + greater
    ...
    // PM sender if true
    if ($pm_sender)
        $db->query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES (" . $sender . ", " . user::$current['uid'] . ", UNIX_TIMESTAMP(), " . $subject . ", " . $msg . ")");
    //pm sent block
...
elseif ($action == "write") {
    ...
    print("\n<form method='post' name='masspm' action='admincp.php?user=" . user::$current["uid"] . "&code=" . user::$current["random"] . "&do=masspm&action=post'>
    ...
    $res1 = $db->query("SELECT id, level FROM users_level WHERE id_level > 1 ORDER BY id_level");
    while ($row1 = $res1->fetch_array(MYSQLI_BOTH)) {
        $select = "<option value='" . (int)$row1["id"] . "'";
        if ($level1 == $row1["id"])
            $select .= "selected='selected'";
        $select .= ">" . security::html_safe($row1["level"]) . "</option>\n";
        print $select;
    }
    print("</select></td></tr>");
