...
if ($PRIVATE_ANNOUNCE) {
	...
	elseif ($rowpid['WT'] > 0) {
		...
		if (intval($rowpid['downloaded']) > 0)
	        ...
	        $added = MCached::get('ann::wait::time::' . $rowpid['id']);
                if ($added === MCached::NO_RESULT) {
		    $res2 = $db->query("SELECT UNIX_TIMESTAMP(data) AS data, uploader FROM namemap WHERE info_hash = '" . $info_hash . "'");
	            $added = $res2->fetch_array(MYSQLI_BOTH);
                    MCached::add('ann::wait::time::' . $rowpid['id'], $added, 900);
                }
...
if (isset($_GET["event"]))
    $event = $db->real_escape_string($_GET["event"]);
else
    $event = '';
...
function verifyTorrent($hash) {
   ...
   $key = 'summary::count::' . $hash;
   $res = MCached::get($key);
   if ($res === MCached::NO_RESULT) {
...
function getRandomPeers($hash, $where = '') {
   ...
   $key = 'peers::count::' . $hash;
   $peercount = MCached::get($key);
   if ($peercount === MCached::NO_RESULT) {
       if ($GLOBALS["NAT"])
       ...
       MCached::add($key, $peercount, 1800);
   }
...
if (!empty($summaryupdate)) {
    ...
    $db->query("UPDATE LOW_PRIORITY summary SET " . substr($stuff, 1) . " WHERE info_hash = '" . $info_hash . "'");
    MCached::del('summary::count::' . $info_hash);
}
