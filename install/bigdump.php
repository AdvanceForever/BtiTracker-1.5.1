...
error_reporting(0);
if (!defined('IN_INSTALL')) die('Hacking attempt!');
...
if (!$error && isset($_REQUEST["uploadbutton"])) {
	if (is_uploaded_file($_FILES["dumpfile"]["tmp_name"]) && ($_FILES["dumpfile"]["error"]) == 0) {
		...
		if (eregi("(\.php|\.php3|\.php4|\.php5)$", $uploaded_filename)) {
			echo ("<p class=\"error\">You may not upload this type of files.</p>\n");
		}
		else
		if (!@move_uploaded_file($_FILES["dumpfile"]["tmp_name"], $uploaded_filepath)) {
...
if (!$error && isset($_REQUEST["delete"]) && $_REQUEST["delete"] != basename($_SERVER["SCRIPT_FILENAME"])) {
	if (@unlink(basename($_REQUEST["delete"]))) echo ("<p class=\"success\">" . $_REQUEST["delete"] . " was removed successfully</p>\n");
...
if (!$error) {
	...
	if (!$db) {
		echo ("<p class=\"error\">Database connection failed due to " . $db->error . "</p>\n");
		echo ("<p>Edit the database settings in " . $_SERVER["SCRIPT_FILENAME"] . " or contact your database provider</p>\n");
		$error = true;
	}

	if (!$error && $db_connection_char_set !== '') @$db->query("SET NAMES $db_connection_char_set", $db);
}
	...
	if ($dirhandle = opendir($upload_dir)) {
		...
		while (false !== ($dirfile = readdir($dirhandle))) {
			if ($dirfile != "." && $dirfile != ".." && $dirfile != basename($_SERVER["SCRIPT_FILENAME"])) {
				...
				if (eregi("\.gz$", $dirfile)) echo ("<td>GZip</td>");
				else echo ("<td>SQL</td>");
				if (!eregi("\.gz$", $dirfile) || function_exists("gzopen")) echo ("<td><a href=\"" . $_SERVER["PHP_SELF"] . "?action=step3&start=1&fn=$dirfile&foffset=0&totalqueries=0\">Start Import</a> into $database at $dbhost</td>\n");
				else echo ("<td>&nbsp;</td>\n");
...
<form method="POST" action="<?php
		echo ($_SERVER["PHP_SELF"]); ?>?action=step3" enctype="multipart/form-data">
...
if (!$error && !isset($_REQUEST["fn"]) && $filename == "") {
	$result = $db->query("SHOW VARIABLES LIKE 'character_set_connection';");
...
if (!$error && isset($_REQUEST["fn"])) {
	...
	if (eregi("\.gz$", $_REQUEST["fn"])) $gzipmode = true;
	else $gzipmode = false;
...
if (!$error && isset($_REQUEST["start"]) && isset($_REQUEST["foffset"])) {
	...
	if (!$error) {
		...
		while ($linenumber < $_REQUEST["start"] + $linespersession || $query != "") {
			...
			if (ereg(";$", trim($dumpline)) && !$inparents) {
				if (!$db->query(trim($query) , $dbconnection)) {
					echo ("<p class=\"error\">Error at the line $linenumber: " . trim($dumpline) . "</p>\n");
					echo ("<p>Query: " . trim($query) . "</p>\n");
					echo ("<p>MySQL: " . $db->error . "</p>\n");
					$error = true;
					break;
				}
	...
	if (!$error) {
		...
		if ($filesize > 0) echo (" or " . round($foffset / $filesize * 100) . "%");
		echo ("</p>\n");
		if ($linenumber < $_REQUEST["start"] + $linespersession) {
			...
			if ($delaypersession != 0) echo ("<p>Now I'm <b>waiting $delaypersession milliseconds</b> before starting next session...</p>\n");
...
if ($error) echo ("<p><input type=\"button\" class=\"button\" name=\"continue\" value=\"Start from the beginning\" onclick=\"javascript:document.location.href='" . $_SERVER["PHP_SELF"] . "?action=step3'\"> (DROP the old tables before restarting)</p>\n");

if ($db) $db->close();

if ($file && !$gzipmode) fclose($file);
else
if ($file && $gzipmode) gzclose($file);
?>
</td></tr></table>
</body>
</html>
